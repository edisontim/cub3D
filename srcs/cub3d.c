...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/05/18 11:27:16 by tedison           #+#    #+#             */
/*   Updated: 2021/06/02 14:56:04 by tedison          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	cub3d(t_p *p, int argc, char *argv[])
{
	...
	if (p->pos_x == 0 && p->pos_y == 0)
		return (ft_error_map(&p->g_p, 1, NULL) & ft_error("No starting pos."));
	if (!check_params(p))
		return (0);
	p->mlx = mlx_init();
...
int	main(int argc, char *argv[])
{
	...
	if (argc == 3)
	{
		if (len(argv[2]) == 6 && ft_strncmp(argv[2], "--save", 6) == 0)
			p.screens = -1;
		else
